...
_locs = ["hh3", "sy1", "ff1", "os5", "fd2", "va1", "sg2", "kw1"]
...
def create_and_populate(rows):
    ...
    with con:
        ...
        for row in rows:
            try:
                cur.execute(config.get_realtime_slack_insert_query(row[2]), tuple(row))
...
def truncate():
    ...
    with con:
        ...
        for row in _locs:
            try:
                cur.execute(config.defaults.realtime_truncate_query.format(row))
            except Exception as e:
                print("row is not truncated" + str(row))
                print(e)

def main():
    slack_token = "xoxp-4188758260-214448774358-266149358231-638d2c7559cd9e1e16b63466c6a08fc7"
    channel_name = 'storage_sdp_prod_mon'
    ...
    truncate() # truncate realtime tables
